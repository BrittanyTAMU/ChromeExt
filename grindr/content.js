// Human.js-powered MatchGuard Content Script - Grindr Edition
let Human;
let human;
...
let swipeCount = 0;
const maxSwipes = 100; // Grindr: 100 swipes per day (free tier)
...
async function autoScrollAndScan() {
  ...
  while (autoScrollActive && swipeCount < maxSwipes && !matchFound) {
    ...
    if (scanResult.match) {
      ...
      await sendEmailAlert('A potential match was found on Grindr!');
  ...
  if (!matchFound) {
    const message = `Scan completed! No match found after ${swipeCount} profiles. Rescan if you have premium access on Grindr.`;
